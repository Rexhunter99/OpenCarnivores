...
#include <d3d11.h>
//#include "d3dx11.h" // Fuck you too Microsoft!
//#include <d3dcsx.h>
...
public:
	...
	VertexBufStruct(float X = 0.f, float Y = 0.f, float Z = 0.f, float S = 0.f, float T = 0.f, int R = 0, int G = 0, int B = 0, int A = 255) :
		x(X), y(Y), z(Z),
		s(S), t(T),
		r(R), g(G), b(B), a(A)
	{}
...
/** Local RenderD3D11.cpp variables **/
...
ID3D11Buffer* g_VertexBufferG = nullptr;
D3D11_MAPPED_SUBRESOURCE g_VertexBufferMap;

std::vector<VertexBufStruct> g_VertexBufferData;

bool DIRECT3DACTIVE = false;
...
{
	HARD3D = true;
	std::cout << "==== Initialising Direct3D 11 Hardware ====" << std::endl;
